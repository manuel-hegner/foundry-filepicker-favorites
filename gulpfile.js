		...
		for (const file of statics) {
			if (fs.existsSync(path.join('src', file))) {
				if(file.endsWith('.json'))
					await gulp.src(path.join('src', file)).pipe(stripJsonComments()).pipe(gulp.dest('dist'))
				else
 ...
 */
function buildWatch() {
	let options = {usePolling: true, ignoreInitial: false}
	gulp.watch('src/**/*.ts', options, buildTS);
	gulp.watch('src/**/*.js', options, buildJS);
	gulp.watch('src/**/*.less', options, buildLess);
	gulp.watch('src/**/*.scss', options, buildSASS);
	gulp.watch(
